...
/- Here is a version of the pigeonhole principle that
concerns functions from Fin n to S where n > |S|. It gives a conclusion with i < j mapping to same
element of S.
-/

/-
The warning is that this makes no use of the section variables, and indeed, it doesn't matter here
that S is a semigroup. But should we change it ?
-/

theorem exists_lt_map_eq_of_card_lt [Fintype S] (n : ℕ+)
 (f : Fin n → S) (h : Fintype.card S < n) :
  (∃ i j : Fin n, i < j ∧ f i = f j) := by
     have : ∃ i j : Fin n, i ≠ j ∧ f i = f j := by
      apply Fintype.exists_ne_map_eq_of_card_lt
      calc
        Fintype.card S < n := h
        _              = Fintype.card (Fin (n:ℕ)) :=  Eq.symm (Fintype.card_fin ↑n)
     obtain ⟨ ii, jj, hne , hf ⟩ := this
     let i := min ii jj; let j := max ii jj
     use i, j
     constructor
     · exact inf_lt_sup.mpr hne
     · have hori : i = ii ∨ i = jj := Std.MinEqOr.min_eq_or ii jj
       have horj : j = ii ∨ j = jj := Std.MaxEqOr.max_eq_or ii jj
       rcases hori with hi1 | hi2
       · rcases horj with hj1 | hj2
         · rw [hi1, hj1]
         · rw [hi1, hj2]
           exact hf
...
lemma exists_repeating_ppow_fintype [Fintype S] (n : ℕ+) (x : S) :
 ∃ m : ℕ+, m ≤ Fintype.card S ∧ x^m = x^n := by
  ...
  · have hexop : ∃ o p : (Fin  n), o < p ∧ x ^ (incnat (o : ℕ) ) = x ^(incnat (p : ℕ) ) := by
      apply exists_lt_map_eq_of_card_lt
    ...
    · let diff := n - (incnat (p : ℕ ))
      ...
      have : incnat (o : ℕ) + diff < n := by
        ...
        calc
          ...
          _ = n := hpdiffeqn
      have : ∃ (m : ℕ+), m ≤ Fintype.card S ∧
      x^m = x ^ (incnat (o : ℕ) + diff ) := ih (incnat (o:ℕ ) + diff) this
      obtain ⟨m,h₁,h₂⟩ := this
      use m
      constructor
      · exact h₁
