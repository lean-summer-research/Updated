...
/-- If `m < n` for positive naturals, then `n = k + m` for some positive `k`. -/
lemma exists_eq_add_of_lt (m_lt_n : m < n) : ∃ (k : ℕ+), n = k + m := by
  exact ⟨n - m, by pnat_to_nat; omega⟩

/-- For positive naturals, `m + (n - m) = n` for `m < n`. -/
@[simp] lemma add_sub_cancel' (m_lt_n : m < n) : m + (n - m) = n := by pnat_to_nat; omega
